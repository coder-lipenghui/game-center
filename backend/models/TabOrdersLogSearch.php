<?php

namespace backend\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\TabOrdersLog;

/**
 * TabOrdersLogSearch represents the model behind the search form of `backend\models\TabOrdersLog`.
 */
class TabOrdersLogSearch extends TabOrdersLog
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'gameid', 'distributor', 'gameServerId', 'isDebug'], 'integer'],
            [['orderid', 'distributorOrderid', 'player_id', 'gameRoleid', 'gameRoleName', 'gameAccount', 'payTime', 'orderTime', 'deviceId'], 'safe'],
            [['total', 'vcoinRatio', 'paymoney'], 'number'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = TabOrdersLog::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'gameid' => $this->gameid,
            'distributor' => $this->distributor,
            'gameServerId' => $this->gameServerId,
            'total' => $this->total,
            'vcoinRatio' => $this->vcoinRatio,
            'paymoney' => $this->paymoney,
            'payTime' => $this->payTime,
            'orderTime' => $this->orderTime,
            'isDebug' => $this->isDebug,
        ]);

        $query->andFilterWhere(['like', 'orderid', $this->orderid])
            ->andFilterWhere(['like', 'distributorOrderid', $this->distributorOrderid])
            ->andFilterWhere(['like', 'player_id', $this->player_id])
            ->andFilterWhere(['like', 'gameRoleid', $this->gameRoleid])
            ->andFilterWhere(['like', 'gameRoleName', $this->gameRoleName])
            ->andFilterWhere(['like', 'gameAccount', $this->gameAccount])
            ->andFilterWhere(['like', 'deviceId', $this->deviceId]);

        return $dataProvider;
    }
}
